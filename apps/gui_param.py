#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Interface graphique — modèle enrichi HEAT-COND (géométrie + matériau).

Application Tkinter. Trois onglets :
  1. Calcul ponctuel  : Q, J, coût, masse pour un design (matériau + t + l par
     ailette, Bi).
  2. Optimisation     : 3 méthodes (NM / DE / Adam→L-BFGS) + multi-fidélité (DE grossier ->
     raffinement fin), objectif F = Q − λ_coût·coût − λ_masse·masse.
  3. Visualisation    : maillage + champ T (matplotlib intégré).

Style et patterns : thread + barre de progression pour ne pas figer l'UI.
"""
...
sys.path.insert(0, str(HERE.parent))  # racine du dépôt pour `import heatcond`
...
class EnrichedGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("HEAT-COND enrichi — géométrie + matériau")
    ...
    def _build_header(self):
        ...
        ttk.Label(inner, text="HEAT-COND — Modèle enrichi (géométrie + matériau)",
                  style="Title.TLabel").pack(anchor="w")
        ttk.Label(inner, text="Objectif Q = chaleur dissipée  |  F = Q − λ_coût·coût − λ_masse·masse",
    ...
    def _build_tab_eval(self):
        ...
        df = ttk.LabelFrame(tab, text="Design des ailettes (matériau / épaisseur t / longueur l)",
                            padding=12)
        df.grid(row=0, column=0, sticky="new")
        ttk.Label(df, text="Ailette", style="Muted.TLabel").grid(row=0, column=0, padx=6)
        ttk.Label(df, text="Matériau", style="Muted.TLabel").grid(row=0, column=1, padx=6)
        ...
        # Bi + mesh
        pf = ttk.LabelFrame(tab, text="Condition & maillage", padding=12)
    ...
    def _build_tab_opt(self):
        ...
        sf = ttk.LabelFrame(tab, text="Objectif & budget", padding=12)
        ...
        for c, (lbl, var) in enumerate([("maillage grossier", self.coarse_var),
                                        ("maillage fin", self.fine_var),
        ...
        ttk.Label(mf, text="DE sur maillage grossier → raffinement Nelder-Mead (matériaux figés) sur maillage fin.",
                  style="Muted.TLabel").grid(row=1, column=0, columnspan=8, sticky="w", pady=(6, 0))
        self.method_var = tk.StringVar(value="Multi-fidélité (DE → NM)")
        ttk.Label(mf, text="Méthode :").grid(row=2, column=0, sticky="w", pady=(8, 0))
        ttk.Combobox(mf, textvariable=self.method_var, state="readonly", width=26,
                     values=["Multi-fidélité (DE → NM)", "Nelder-Mead",
                             "Differential Evolution", "Adam → L-BFGS"]).grid(
                     row=2, column=1, columnspan=3, sticky="w", padx=6, pady=(8, 0))
        ttk.Label(mf, text="Mêmes trois méthodes que la Partie I. Les méthodes simples tournent sur le maillage fin ; "
                  "matériaux relâchés + arrondis.", style="Muted.TLabel").grid(
        ...
        ttk.Button(bf, text="Charger ce design dans « Calcul ponctuel »",
                   command=self.load_best_into_eval).pack(side=tk.LEFT, padx=8)
        ttk.Button(bf, text="Afficher T optimisé",
        ...
        rf = ttk.LabelFrame(tab, text="Meilleur design", padding=12)
    ...
    def _build_tab_viz(self):
        ...
        for i, (txt, val) in enumerate([
                ("Maillage (design courant)", "mesh"),
                ("Champ T (calcul ponctuel)", "T_current"),
                ("Champ T (optimisé)", "T_opt")]):
        ...
        ttk.Button(bb, text="Actualiser", style="Accent.TButton",
                   command=self.refresh_viz).pack(side=tk.LEFT)
        ttk.Button(bb, text="Sauvegarder PNG…", command=self.save_viz).pack(side=tk.LEFT, padx=8)
        ...
        self._placeholder("Lance un calcul ou une optimisation, puis « Actualiser ».")
    ...
    # =======================================================================
    # Lecture / validation des entrées
    # =======================================================================
    def _read_design(self):
        try:
            ...
            messagebox.showerror("Erreur", "Entrées invalides (nombres attendus).")
            return None
    ...
    def calc_point(self):
        ...
        self._set_status("Calcul ponctuel : FreeFEM…")
        self._busy(True)
    ...
    def _do_calc(self, d):
        ...
        try:
            ...
            msg = (f"Q = {Q:.6f}   (chaleur dissipée — objectif de performance)\n"
                   f"J = {J:.6f}   (température moyenne — info)\n"
                   f"Coût = {cost:.4f}   |   Masse = {mass:.4f}\n"
                   f"Matériaux : {names}\n"
    ...
    def reset_design(self):
        ...
        self._set_status("Design par défaut restauré.")
    ...
    def run_optimization(self):
        try:
            ...
            messagebox.showerror("Erreur", "Paramètres d'optimisation invalides.")
            return
        if not messagebox.askyesno(
                "Optimisation",
                f"Méthode : {method}\nλ_coût={lam_cost}, λ_masse={lam_mass}, "
                f"budget_masse={mass_budget}\nmaillage {coarse}→{fine}, "
                f"maxiter={maxiter}, popsize={popsize}\n\nLancer ?"):
            return
        self.opt_result_var.set("en cours…")
        self._set_status("Optimisation en cours…")
        self._busy(True)
        self.progress["maximum"] = maxiter
        self.progress["value"] = 0
        self.progress_label.config(text=f"0 / {maxiter} générations (DE grossier)")
        threading.Thread(
            target=self._do_opt,
    ...
    def _do_opt(self, method, lam_cost, lam_mass, mass_budget, coarse, fine, maxiter, popsize):
        ...
        try:
            if method.startswith("Multi"):
                ...
                extra = (f"DE grossier : {glob['n_eval']} évals, {glob['time']:.0f}s  →  "
                         f"raffinement fin : {res['n_eval']} évals, {res['time']:.0f}s")
                mesh_used = fine
            else:
            ...
            msg = (f"Méthode : {method}\n"
                   ...
                   f"Coût = {res['best_cost']:.4f}   |   Masse = {res['best_mass']:.4f}\n"
                   f"{extra}\n\n"
    ...
    def _update_progress(self, done, total):
        ...
        self.progress_label.config(text=f"{done} / {total} générations (DE grossier)")

    def load_best_into_eval(self):
        if self.last_best_x is None:
            messagebox.showinfo("Aucun design", "Lance d'abord une optimisation.")
            return
        ...
        self._set_status("Meilleur design chargé dans « Calcul ponctuel ».")
        ...
        try:
            if kind == "mesh":
                ...
                self._viz_T(self.last_T_file, self.last_mesh_path,
                            "Champ T (calcul ponctuel)")
            elif kind == "T_opt":
                self._viz_T(self.last_opt_T, self.last_opt_mesh,
                            "Champ T (design optimisé)")
    ...
    def _viz_mesh(self):
        ...
        if mp is None or not Path(mp).exists():
            raise FileNotFoundError("Aucun maillage — lance d'abord un calcul.")
    ...
    def _viz_T(self, t_file, mesh_path, title):
        if t_file is None or not Path(t_file).exists():
            raise FileNotFoundError("Champ T indisponible — lance d'abord le calcul correspondant.")
    ...
    def save_viz(self):
        path = filedialog.asksaveasfilename(
            title="Sauvegarder la figure", defaultextension=".png",
